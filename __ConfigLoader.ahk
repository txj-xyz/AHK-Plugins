...
; Version:   1.0.1
; ---------------------------
; Load a JSON file located at
; your projects root dir under
; 'config\config.json' file.
; this will load a variable 
; under the name of 'config'
; globally for you to use.
; ---------------------------
; Example Usage
; ---------------------------
; MsgBox % "Can you see this '" . config.str . "' variable?"
; ---------------------------

; Export global values
config_path := A_ScriptDir . "\config\config.json"

; If the file exists then we start loading up the JSON files
if FileExist(config_path) {
    FileRead, fileContent, % config_path
    global config := JSON.parse(fileContent)
} else {
    MsgBox % "[ERROR] Configuration file not found: " . config_path
}

; Reload the client configuration JSON file
__reloadConfig() {
    config_path := A_ScriptDir . "\config\config.json"
    if FileExist(config_path) {
        FileRead, fileContent, % config_path
        global config := JSON.Load(fileContent)
        return config
    } else {
        MsgBox % "[ERROR] Configuration file not found: " . config_path
        return False
...
; --------- @@@@@@@@@@@ DEPENDENCY LOADER @@@@@@@@@@@ ---------
; @@@@@@@@@@@ DEPENDENCY LOADER @@@@@@@@@@@
; Script:    _JSONLoader.ahk
; License:   Custom (BSD 2-Clause)
; Author:    https://github.com/Chunjee/json.ahk
; -------------------------------------------------------------
class JSON
{
